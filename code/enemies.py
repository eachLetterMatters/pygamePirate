...
class Tooth(pygame.sprite.Sprite):
    ...
    def update(self, dt):
        ...
        detect_wall_rect = pygame.FRect(self.rect.topleft + vector(-1,0),(self.rect.width + 2, 1))
        ...
        if floor_rect_right.collidelist(self.collision_rects) < 0 and self.direction > 0 or \
            floor_rect_left.collidelist(self.collision_rects) < 0 and self.direction < 0  or \
                detect_wall_rect.collidelist(self.collision_rects) != -1:
...
class Pearl(pygame.sprite.Sprite):
    def __init__(self, pos, groups, surf, direction, speed):
        self.pearl = True   # used in hasattr to destroy upon collision with player
        super().__init__(groups)
        self.image = surf
