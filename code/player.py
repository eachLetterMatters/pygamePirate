...
class Player(pygame.sprite.Sprite):
	...
	def move(self, dt):
		...
		# vertical
		if not self.on_surface['floor'] and any((self.on_surface['left'], self.on_surface['right'])) and not self.timers['wall slide block'].active:	# wall slide
			self.direction.y = 0
		...
		if self.jump:
			...
			elif any((self.on_surface['left'], self.on_surface['right'])) and not self.timers['wall slide block'].active:	# wall jump
	...
	def platform_move(self, dt):	# method created to fine tune collisions with platforms
		if self.platform:
	...
	def check_contact(self):
		...
		# collisions
		# check floor collision with semi collidable only if player is moving down (direction.y >= 0)
		self.on_surface['floor'] = True if floor_rect.collidelist(collide_rects) >= 0 or floor_rect.collidelist(semi_collide_rects) >= 0 and self.direction.y >= 0 else False	# collidelist method
		self.on_surface['right'] = True if right_rect.collidelist(collide_rects) >= 0 else False
		self.on_surface['left'] = True if left_rect.collidelist(collide_rects) >= 0 else False

		# assign platform collided with to fine tune movement
		self.platform = None
	...
	def collision(self, axis):
		for sprite in self.collision_sprites:
			if sprite.rect.colliderect(self.hitbox_rect):
				# wrap old rects in int to solve collision issues
				if axis == 'horizontal':
					# left
					...
					if self.hitbox_rect.top <= sprite.rect.bottom and int(self.old_rect.top) >= int(sprite.old_rect.bottom):
						...
						self.direction.y = 0
						if hasattr(sprite, 'moving'): # if collision with moving platform
	...
	def semi_collision(self):
		if not self.timers['platform skip'].active:
			for sprite in self.semi_collision_sprites:
				if sprite.rect.colliderect(self.hitbox_rect):
					# bottom collision, wrap in int to solve collision issues
